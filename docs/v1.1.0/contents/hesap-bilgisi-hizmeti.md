...
|>> Güncel Bakiye Tutarı |	gnclBky|	AN1..25|	İ	|İşlem sonrası güncel bakiye bilgisi tutarı. Bu alan Sürüm 2.0.0'da Z-Zorunlu olarak iletilecektir.  <br> Tutar alanı regex patterni şu şekildedir: '```^[\-\+]?\d{1,18}$\|^[\-\+]?\d{1,18}\\.\d{1,5}$```'|
|>> Para Birimi	|prBrm|	AN3|	Z|	Para birimi.|
